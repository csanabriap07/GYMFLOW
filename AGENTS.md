...
- **Navegación del backoffice:** las páginas nuevas se registran en `frontend/src/navigation/staffMenu.ts` (un `StaffMenuItem` con su `path` y el `permission`/rol que hace falta para verla) — no se linkean a mano en otros componentes. `StaffHome` (`/staff/home`) arma el menú filtrando ese registro por lo que puede hacer el usuario logueado.
